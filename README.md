...
> ⚠️ **注意**: デモのバックエンドは低スペックの Render プラン上で動作しているため、**画像アップロードによる実検査はメモリ不足で失敗します**（特に大きな画像や SAM 利用時）。本デモは主に **UI / 表示のモックアップ確認用** です。実際の解析結果は、下記「セットアップ」に従ってローカルで実行することで確認できます。実行結果の見え方は「解析結果の例」のスクリーンショットを参照してください。
