...
🔗 **<https://cloth-wrinkle-analyzer.vercel.app/>**

フロントエンドを Vercel、バックエンドを Render にホストしたデモ環境です。UI と解析結果の表示を確認できます。

> ⚠️ **注意**: デモのバックエンドは低スペックの Render プラン上で動作しているため、**画像アップロードによる実検査はメモリ不足で失敗することがあります**（特に大きな画像や SAM 利用時）。本デモは主に **UI / 表示のモックアップ確認用** です。実際の解析を試す場合は、下記「セットアップ」に従ってローカルで実行することを推奨します。実行結果の見え方は「解析結果の例」のスクリーンショットを参照してください。
